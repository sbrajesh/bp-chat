...
function is_user_logged_in( $user_id ){
    ...
    $query = $wpdb->prepare( "SELECT user_id FROM  {$bpchat->table_name_users} where is_online = %d AND user_id = %d", 1, $user_id );
...
//get current online users count
  function get_online_users_count(){
      ...
      $total_users_sql = "SELECT DISTINCT COUNT(user_id) FROM {$bpchat->table_name_users} WHERE is_online=1 and user_id!=%d";
...
//clean the logged in user table
  function cleanup(){
      ...
      $query="UPDATE {$bpchat->table_name_users} SET is_online=%d where DATE_ADD( last_active_time, INTERVAL 2 MINUTE ) <= NOW()";
